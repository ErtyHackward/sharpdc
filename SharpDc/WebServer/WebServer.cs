    ...
    public class WebServer : IDisposable
    {
        ...
        public string LocalIPAddress
        {
            get
            {
                ...
                {
                    if (ip.AddressFamily.ToString() == "InterNetwork" && (ip.ToString().StartsWith("10") || ip.ToString().StartsWith("46.180") || ip.ToString().StartsWith("95")))
                    {
                        localIP = ip.ToString();
                        break;
                    }
                ...
                while (_listener != null)
                {
                    _listener.BeginAcceptTcpClient(delegate(IAsyncResult result)
                    {
                        try
                        {
                            ...
                            _autoReset.Set();
                            var ea = new WebServerConnectedEventArgs(client);
                            OnConnected(ea);
                            if (!ea.Handled)
                            {
                                ...
                                var rea = new WebServerRequestEventArgs(req, resp);
                                OnRequest(rea);
                                resp.Close();
                                client.Close();
                            }
