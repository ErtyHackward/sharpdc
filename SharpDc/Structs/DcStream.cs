...
namespace SharpDc.Structs
{
    ...
    public class DcStream : Stream
    {
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            if (_position + offset + count > Magnet.Size)
            {
                Logger.Warn("Requested data after the length of the file, trimming [fileLen:{0}, pos:{1}, offset:{2}, count:{3}]", Magnet.Size, _position, offset, count);
                count = (int)(Magnet.Size - (_position + offset));

                if (count <= 0)
                    return 0;
            }
            
            using (new PerfLimit(string.Format("DcStream read pos:{0} len:{1}", _position + offset, count), 500))
            {
                if (_fileStream != null)
                {
